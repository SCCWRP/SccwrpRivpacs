SFBayRivpacs <- function(Pcutoff = 0.5,
  ...
  predictor.variables <- c("SampleDepth", "Hab_G", "Longitude")
  
  # ----- Format Observed Data -----
  
  FormatObservedData <- function() {
    
    # Align observed data columns with reference data columns. Columns in same
  ...
  CalculateScores <- function() {
    ...
    # Container.
    #   OE.stats <- data.frame(observed.score = rep(NA, expected.data$n), 
    #                          expected.score = rep(NA, expected.data$n), 
    #                          BC = rep(NA, expected.data$n) # Bray-Curtis dissimilarity 
    #   )
