...
ts <- "TestMultipleStations"
uf <- "R CMD check Read CSV Test Multiple Stations"

# Read in user files.

station <- read.csv(paste(system.file(package = "SccwrpRivpacs"), "/extdata/TestMultipleStationInfo.csv", sep = ""), stringsAsFactors = FALSE) 

benthic <- read.csv(paste(system.file(package = "SccwrpRivpacs"), "/extdata/TestMultipleBenthicInfo.csv", sep = ""), stringsAsFactors = FALSE) 
